...
export type categoryFilterNameType =
  | "All"
  | "New"
  | "Ghee"
  | "Oils"
  ...
  | "₹499"
  | "₹999";
export type categoryFilterSlugType =
  | "all-products"
  | "newly-launched"
  | "ghee"
  | "oils"
...
const CategoryPanel = ({ activePath }: { activePath: string }) => {
  ...
  return (
    <div className="w-full overflow-x-auto no-scrollbar">
      <div className="flex gap-6 sm:gap-8 md:gap-10 px-1 py-2 min-w-max items-center justify-center">
        {categoryFilters.map((f) => (
          ...
          >
            <img
              className="w-14 md:w-16 h-14 md:h-16 hover:scale-105 transition-transform ease-in-out duration-100"
              src={
                activeCategoryName === f.name ? f.active_src : f.inactive_src
              }
              alt="cat"
            />
            {f.name}
