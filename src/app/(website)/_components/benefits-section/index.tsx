...
import { GoldDivider, SectionLabel, SectionTitle } from "../../page";
          ...
          {items.map(({ icon: Icon, title, desc }) => (
            <div key={title} className="flex flex-col items-center text-center group">
              <div
                ...
                style={{
                  background: `linear-gradient(135deg, ${GREEN}12, ${GOLD}10)`,
                  border: `1.5px solid ${GOLD}30`,
                }}
              >
                <Icon size={24} style={{ color: GREEN }} />
              </div>
              <h4 className="font-bold text-base mb-2" style={{ color: "#111827", fontFamily: LATO }}>{title}</h4>
              <p className="text-sm leading-relaxed" style={{ color: "#9ca3af", fontFamily: LATO }}>{desc}</p>
            </div>
          ))}
        </div>
      </div>
